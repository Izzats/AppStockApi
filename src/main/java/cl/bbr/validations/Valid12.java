...
public class Valid12 {
	...
	private static String TIPO_DOCUMENTO = "G";
	private static int CANTIDAD_CAMPOS = 16;
	...
	private static int LARGO_NOGUIDES = 10;
	private static int LARGO_NUM_VENDEDOR = 10;
	private static int LARGO_MARC_PDCTO = 50;
	
	public static Response isValid12(Trx venta) {
...
//		System.out.println("canal "+trxId[1].subSequence(0, 2));
		...
		if (trxId[13].length() > LARGO_NOGUIDES) {
			logger.error("Número de Guía de Despacho inválido: " + trxId[13]);
			message.setResponseError("37", "Número de Guía de Despacho inválido: " + trxId[13]);
			return message;
		}
		
		if (trxId[14].length() > LARGO_NUM_VENDEDOR) {
			logger.error("Número de vendedor inválido: " + trxId[14]);
			message.setResponseError("36", "Número de vendedor inválido: " + trxId[14]);
			return message;
		}
		
		if (trxId[15].length() > LARGO_MARC_PDCTO) {
			logger.error("Marca de productos inválida: " + trxId[15]);
			message.setResponseError("35", "Marca de productos inválida: " + trxId[15]);
			return message;
		}
