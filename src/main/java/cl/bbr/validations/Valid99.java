...
public class Valid99 {
	...
	private static String TIPO_DOCUMENTO = "NE";
	private static int CANTIDAD_CAMPOS = 26;
	...
	private static int LARGO_NRO_DOC = 10;
	private static int LARGO_NRO_LOCAL = 4;  
	...
	private static int LARGO_RAZON_REF = 30; 
	private static int LARGO_DOC_NC = 10;
	private static int LARGO_LOCAL_OR = 4; 
	private static int LARGO_POS_OR = 4; 
	private static int LARGO_NOTRX_OR = 4; 
	private static int LARGO_FECHA_DOCOR = 8; 
	private static int LARGO_HORA_DOCOR = 8; 
	private static int LARGO_NO_TRXNC = 8;
	
	public static Response isValid99(Trx venta) {
...
//		System.out.println("canal "+trxId[1].subSequence(0, 2));
		...
		if (!trxId[3].equals(TIPO_DOCUMENTO) || trxId[3].length() > LARGO_TIPO_DOCUMENTO) {
		...
		if (trxId[12].length() > LARGO_NRO_DOC) {
			logger.error("Número de documento inválido " + trxId[12]);
			message.setResponseError("20", "Número de documento inválido " + trxId[12]);
			return message;
		}
		
		if (trxId[13].length() > LARGO_NRO_LOCAL) {
			logger.error("Número de local inválida" + trxId[13]);
			message.setResponseError("21", "Número de local inválida" + trxId[13]);
			return message;
		}
		
		if (trxId[14].length() > LARGO_FECHA_TRX) {
			logger.error("Fecha TRX inválida " + trxId[14]);
			message.setResponseError("22", "Fecha TRX inválida " + trxId[14]);
			return message;
		} 
		
		if (trxId[15].length() > LARGO_HORA_TRX) {
			logger.error("Hora TRX inválida " + trxId[15]);
			message.setResponseError("23", "Hora TRX inválida " + trxId[15]);
			return message;
		}
		
		if (trxId[16].length() > LARGO_FECHA_CONT) {
			logger.error("Fecha contable inválida " + trxId[16]);
			message.setResponseError("24", "Fecha contable inválida " + trxId[16]);
			return message;
		}
		
		if (trxId[17].length() > LARGO_NUM_CAJERO) {
			logger.error("Número de cajero inválido " + trxId[17]);
			message.setResponseError("25", "Número de cajero inválido " + trxId[17]);
			return message;
		}
		
		if (trxId[18].length() > LARGO_RAZON_REF) {
			logger.error("Razón referencia inválida " + trxId[18]);
			message.setResponseError("26", "Razón referencia inválida " + trxId[18]);
			return message;
		}
		
		if (trxId[19].length() > LARGO_DOC_NC) {
			logger.error("Documento origen de NC inválido " + trxId[19]);
			message.setResponseError("27", "Documento origen de NC inválido " + trxId[19]);
			return message;
		}
		
		if (trxId[20].length() > LARGO_LOCAL_OR) {
			logger.error("Local origen inválido " + trxId[20]);
			message.setResponseError("28", "Local origen inválido " + trxId[20]);
			return message;
		}   
		
		if (trxId[21].length() > LARGO_POS_OR) {
			logger.error("Pos origen inválido " + trxId[21]);
			message.setResponseError("29", "Pos origen inválido " + trxId[21]);
			return message;
		}
		
		if (trxId[22].length() > LARGO_NOTRX_OR) {
			logger.error("Número TRX origen inválido " + trxId[22]);
			message.setResponseError("30", "Número TRX origen inválido " + trxId[22]);
			return message;
		}
		
		if (trxId[23].length() > LARGO_FECHA_DOCOR) {
			logger.error("Fecha documento origen inválida " + trxId[23]);
			message.setResponseError("31", "Fecha documento origen inválida " + trxId[23]);
			return message;
		} 
		
		if (trxId[24].length() > LARGO_HORA_DOCOR) {
			logger.error("Hora documento origen inválida " + trxId[24]);
			message.setResponseError("32", "Hora documento origen inválida " + trxId[24]);
			return message;
		}
		
		if (trxId[25].trim().length() > LARGO_NO_TRXNC) {
			logger.error("Número TRX de la NC inválido " + trxId[25]);
			message.setResponseError("33", "Número TRX de la NC inválido " + trxId[25]);
			return message;
		}
